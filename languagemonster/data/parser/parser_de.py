...
class ParserDe(Parser):
    ...
    def _get_definition(self, raw, languages):
        ...
        for i, lang in enumerate(languages):
            if i == len(languages) - 1:
                ...
                r = re.findall(
                    r'^==.*?\{\{Sprache\|' + lang + '\}\}.*?==(.*)==.*?\{\{Sprache\|' + languages[i + 1] + '\}\}.*?==',
                    raw,
                    re.M | re.I | re.S | re.UNICODE
                )
    ...
    def _analyse_article(self, raw):
        ...
        for lang, val in defs.iteritems():
            ...
            for line in val.split('\n'):
                r3 = re.findall(r'^===.*?\{\{[\w\s]*?\|([\w\s]+)\|[\w\s]*?\}\}.*?===.*?$', line, re.M | re.I | re.S | re.UNICODE)
    ...
    def _parse(self, word, lang, item):
        ...
        if item.line.startswith(':'):
            ...
            p = parse_link(rm(text, [paren, brack, apos])).strip()
