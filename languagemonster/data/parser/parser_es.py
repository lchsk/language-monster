...
# remove numbers such as: [1], [25] etc.

REMOVE_BRACKET_NUMBERS = False
...
class ParserEs(Parser):
    ...
    def _get_languages(self, raw):
        languages = []
        # == {{lengua|gd}} ==
        a = re.findall(r'^==.*?\{\{lengua\|([\w ]+)\}\}.*?==$', raw, re.M | re.I | re.S | re.UNICODE)
    ...
    def _get_definition(self, raw, languages):
        ...
        for i, lang in enumerate(languages):
            if i == len(languages) - 1:
                ...
                r = re.findall(r'^==.*?\{\{lengua\|' + lang + '\}\}.*?==(.*)', raw, re.M | re.I | re.S | re.UNICODE)
            else:
                
                # several languages
                
                r = re.findall(r'^==.*?\{\{lengua\|' + lang + '\}\}.*?==(.*)==.*?\{\{lengua\|' + languages[i + 1] + '\}\}.*?==', raw, re.M | re.I | re.S | re.UNICODE)
    ...
    def _analyse_article(self, raw):
        ...
        for lang, val in defs.iteritems():
            ...
            for line in val.split('\n'):
                ...
                r4 = re.findall(r'^\{\{(.+?)\}\}$', line, re.M | re.I | re.S | re.UNICODE)
                if r3:
                    head3 = unicode(r3[0])
                    head4 = ''
                elif r4:
    ...
    def _parse(self, word, lang, item):
        ...
        if startswith('^;.*?\:', item.line):
            text = item.line

            text = rm_re('^;.*?\:', text)

            paren = get_context(text, '(', ')', 1)
            apos = get_context(text, "'", "'", 2)
            comments = '{0}||{1}'.format(paren, apos)
            
            if REMOVE_BRACKET_NUMBERS:
                text = rm_re(r'\[\d+\]', text)

            p = parse_smart(rm(text, [paren, apos])).strip()
            # p = parse_link(rm(text, [paren, apos]), symbol='{', count = 2).strip()
