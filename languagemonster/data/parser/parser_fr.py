...
class ParserFr(Parser):
    ...
    def _get_languages(self, raw):
        languages = []
        # == {{lengua|gd}} ==
        a = re.findall(r'^==.*?\{\{langue\|([\w ]+)\}\}.*?==$', raw, re.M | re.I | re.S | re.UNICODE)
    ...
    def _get_definition(self, raw, languages):
        ...
        for i, lang in enumerate(languages):
            if i == len(languages) - 1:
                ...
                r = re.findall(r'^==.*?\{\{langue\|' + lang + '\}\}.*?==(.*)', raw, re.M | re.I | re.S | re.UNICODE)
            else:
                
                # several languages
                
                r = re.findall(r'^==.*?\{\{langue\|' + lang + '\}\}.*?==(.*)==.*?\{\{langue\|' + languages[i + 1] + '\}\}.*?==', raw, re.M | re.I | re.S | re.UNICODE)
    ...
    def _parse(self, word, lang, item):
        ...
        if item.line.startswith('#'):
            ...
            comments = '{0}||{1}||{2}'.format(paren, brack, apos)
            
            if REMOVE_BRACKET_NUMBERS:
                text = rm_re(r'\[\d+\]', text)

            p = parse_link(rm(text, [paren, brack, apos]), symbol='[', count = 2).strip()
