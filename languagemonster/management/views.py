...
def status(request):
    '''
        Status page.
        Only for MasterAdmin!
    '''

    c = get_context(request)

    # Here it should be safe to get all status information
    # This is available only for logged in superusers

    c['superusers_count'] = len(MonsterUser.objects.filter(
    ...
    # db = get_db()
    # 
    # c['collections'] = sorted(db.collection_names(False))
    # c['dbname'] = db.name
    # c['dbstats'] = db.command('dbstats')
    # 
    # collstats = {}
    # 
    # for coll in c['collections']:
    #     collstats[coll] = db.command('collstats', coll)
    # 
    # c['collstats'] = collstats
    ...
    t_5min = datetime.datetime.now() - datetime.timedelta(minutes=5)
    c['users_5min'] = len(MonsterUser.objects.filter(user__last_login__gt=t_5min))

    t_1h = datetime.datetime.now() - datetime.timedelta(hours=1)
    c['users_1h'] = len(MonsterUser.objects.filter(user__last_login__gt=t_1h))

    t_24h = datetime.datetime.now() - datetime.timedelta(hours=24)
    c['users_24h'] = len(MonsterUser.objects.filter(user__last_login__gt=t_24h))

    c['users'] = len(MonsterUser.objects.all())

    # Let's check if celerybeat daemon is running
    ...
    # Writable avatars folder

    c['AVATARS_WRITABLE'] = os.access(
        settings.PROJECT_ROOT + settings.STATIC_URL + settings.AVATARS_URL,
        os.W_OK
    )

    # From settings.py
    c['ID'] = settings.ID
    c['BASE_DIR'] = settings.BASE_DIR
    c['PROJECT_ROOT'] = settings.PROJECT_ROOT
    c['DEBUG'] = settings.DEBUG
    c['THUMBNAIL_FORMAT'] = settings.THUMBNAIL_FORMAT
    c['ALLOWED_HOSTS'] = settings.ALLOWED_HOSTS
    c['INSTALLED_APPS'] = settings.INSTALLED_APPS
    c['USE_I18N'] = settings.USE_I18N
    c['LANGUAGE_CODE'] = settings.LANGUAGE_CODE
    c['TIME_ZONE'] = settings.TIME_ZONE
    c['USE_L10N'] = settings.USE_L10N
    c['USE_TZ'] = settings.USE_TZ
    c['EMAIL_HOST_USER'] = settings.EMAIL_HOST_USER
    c['STATICFILES_DIRS'] = settings.STATICFILES_DIRS
    c['MEDIA_ROOT'] = settings.MEDIA_ROOT
    c['AVATARS_ROOT'] = settings.PROJECT_ROOT + settings.STATIC_URL + settings.AVATARS_URL
    c['LOCALE_PATHS'] = settings.LOCALE_PATHS
    c['REGISTRATION_CONFIRMATION'] = settings.REGISTRATION_CONFIRMATION
    c['CELERYBEAT_SCHEDULE'] = settings.CELERYBEAT_SCHEDULE

    # Check tmp celery files
    try:
        import glob
        log_files = glob.glob('/tmp/*.log')

        c['LOGS'] = {}
        c['LOGS_COUNT'] = len(log_files)
    ...
    c['BASE_LANGUAGES'] = base_languages

    # Language Pairs

    language_pairs = LanguagePair.objects.filter(visible=True)

    pairs = {}

    for p in language_pairs:
        if p.base_language not in pairs:
            pairs[p.base_language] = []
        pairs[p.base_language].append(p)

    c['LANGUAGE_PAIRS'] = pairs

    return render(request, "app/management/status.html", c)
